...
# COMPOSITION
# =====================================================
# Read the current slider value for every component except the last one,
# then auto-balance the last component so the mixture sums to 100%.

composition = {}

for gas in gas_list[:-1]:
    composition[gas] = float(st.session_state[f"{gas}_slider"])

sum_of_others = sum(composition.values())
...
composition[last_gas] = remaining

st.session_state[f"{last_gas}_slider"] = remaining
st.session_state[f"{last_gas}_input"] = remaining
...
# =====================================================
# LAST COMPONENT ROW (auto-balanced, controls disabled)
# =====================================================

row_cols = st.columns(col_widths)

row_cols[0].write(last_gas)
row_cols[1].write(mw_lookup[last_gas])
row_cols[2].write(ncv_lookup[last_gas])
row_cols[3].write(gcv_lookup[last_gas])
...
row_cols[6].write(f"{wt_frac[last_gas] * 100:.2f}%")
row_cols[7].write(f"{ncv_contribution[last_gas]:.2f}")
row_cols[8].write(f"{gcv_contribution[last_gas]:.2f}")
